...
class Dataset_PairedImage(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if self.io_backend_opt["type"] == "lmdb":
            ...
            "meta_info_file" in self.opt
            and self.opt["meta_info_file"] is not None
        ):
...
class Dataset_GaussianDenoising(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if self.io_backend_opt["type"] == "lmdb":
            ...
            icecream.icecream.ic(self.paths)
    ...
    def __getitem__(self, index):
        ...
        # try:
        # print("magic_dict:", self.paths[index]["gt_path"])
        # gt_path = self.paths[index]["gt_path"]

        # except TypeError:
        #     print("human_dict", self.paths[index])
        gt_path = os.path.join(self.paths[index])
        img_bytes = self.file_client.get(gt_path, "gt")
...
class Dataset_DefocusDeblur_DualPixel_16bit(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt["phase"] == "train":
            gt_size = self.opt["gt_size"]
            # padding
            img_lqL, img_lqR, img_gt = padding_DP(
                img_lqL, img_lqR, img_gt, gt_size
            )
            ...
            # flip, rotation
            if self.geometric_augs:
                img_lqL, img_lqR, img_gt = random_augmentation(
                    img_lqL, img_lqR, img_gt
                )
