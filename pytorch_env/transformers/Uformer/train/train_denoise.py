...
opt = (
	options.Options()
	.init(argparse.ArgumentParser(description="Image denoising"))
	.parse_args()
)
print(opt)
...
# 	print(
# 		"Input & GT (PSNR) -->%.4f dB" % (psnr_dataset),
# 		", Model_init & GT (PSNR) -->%.4f dB" % (psnr_model_init),
# 	)
		...
		if (i + 1) % eval_now == 0 and i > 0:
			with torch.no_grad():
				...
				for ii, data_val in enumerate((val_loader), 0):
					...
					restored = torch.clamp(restored, 0, 1)
					psnr_val_rgb.append(
						utils.batch_PSNR(restored, target, False).item()
					)
